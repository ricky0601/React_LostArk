import { ARMLET_POWER_BY_LEVEL, resolveArmletLevel } from '../../data/specScore/equipmentPowerTables';
...
export const buildModifiedSpecScoreData = (
  ...
  for (const slot of SLOT_ORDER) {
    ...
    if (slot === 'armlet') {
      ...
      // 명시적 한계돌파 등급이 최우선이고, 계수 테이블 grade는 레벨이 바뀐 경우에만 쓴다.
      const isLevelChanged = requestedLevel !== current.normalLevel;
      equip[slot] = {
        ...current,
        normalLevel: requestedLevel,
        advancedLevel: 0,
        tier: mod?.armletGrade
          ?? (isLevelChanged && armletLevel !== null
