...
import { ARMLET_POWER_BY_LEVEL, ARMLET_UNEQUIPPED_LEVEL, resolveArmletPower } from '../../data/specScore/equipmentPowerTables';
...
export const fetchSpecScoreRawData = async (profile: CharacterProfile): Promise<SpecScoreRawData> => {
  ...
  const armletPower = resolveArmletPower({
    normalLevel: equip.armlet?.normalLevel ?? ARMLET_UNEQUIPPED_LEVEL,
    grade: equip.armlet?.tier,
