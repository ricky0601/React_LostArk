...
const NicknameSearchBar: React.FC<NicknameSearchBarProps> = ({
  ...
  const nicknameSearchInputId = useId();
  ...
  return (
    <form onSubmit={handleSubmit} role="search" className="flex items-center gap-2 max-w-xs mx-auto">
      <div className="relative flex-1">
        ...
        <label htmlFor={nicknameSearchInputId} className="sr-only">{placeholder}</label>
        <input
          id={nicknameSearchInputId}
          type="text"
          value={input}
          ...
          autoComplete="off"
          enterKeyHint="search"
          className="w-full pl-8 pr-3 min-h-11 text-sm rounded-lg
      ...
      <button
        type="submit"
        className="px-3 min-h-11 text-sm font-medium rounded-lg
