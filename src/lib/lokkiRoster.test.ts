import { fetchSavedLokkiRoster, syncLokkiRoster, toSiblingCharacter } from './lokkiRoster';
...
describe('lokki roster storage', () => {
  ...
  it('sends one atomic sync payload with normalized levels and combat power', async () => {
    ...
    await syncLokkiRoster({ rpc, from: vi.fn() } as never, [{
      ServerName: '루페온',
      CharacterName: '대표캐릭',
      CharacterLevel: 70,
      CharacterClassName: '슬레이어',
    ...
    expect(rpc).toHaveBeenCalledWith('lokki_sync_roster', {
  ...
  it('surfaces sync errors', async () => {
    ...
    await expect(syncLokkiRoster({ rpc: failedRpc, from: vi.fn() } as never, [])).rejects.toThrow('unavailable');
  });
});
