...
import { ARMLET_UNEQUIPPED_LEVEL, resolveArmletPower } from '../data/specScore/equipmentPowerTables';
...
const sumArmletBaseAttackPercent = (
  ...
  const power = resolveArmletPower({
    normalLevel: equipment?.armlet?.normalLevel ?? ARMLET_UNEQUIPPED_LEVEL,
    grade: equipment?.armlet?.tier,
  });
  return power === null ? 0 : power.baseAttackPercent;
};
...
const sumArmletWeaponAttack = (
  ...
  return power === null ? 0 : power.weaponAttack;
};
